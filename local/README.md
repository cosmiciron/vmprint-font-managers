...
The package is modularized into several core files and a bundled asset directory.
...
**`src/LocalFontManager.ts`** — The main class implementation. Implements all eight methods of the `FontManager` interface. The only method with meaningful complexity is `loadFontBuffer`, which resolves a `src` string to an `ArrayBuffer` using a multi-strategy resolution chain.

**`src/utils.ts`** & **`src/DownloadTracker.ts`** — Internal utilities for normalization, cloning, and CLI progress tracking.
